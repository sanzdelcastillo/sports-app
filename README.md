...
- **Game Detail** — scoreboard (running live clock, 0 not blank for live scores) with highlight link when finished, then tabs: Match (goals/cards/subs timeline + stat bars, live refresh every 2 min), Watch (destinations + listed U.S. broadcasts), Lineups, Table, Calendar
- **Calendar on the web** — a home-screen web app can't download a file it made, so the .ics is compressed into a link to `api/ics.js`, which serves it as text/calendar (iOS then offers Add to Calendar). The native shell uses the share sheet instead
...
- **Any league** — the picker's "All leagues" tab searches the feed's ~700 soccer leagues by league or country name (demonym-aware: "Denmark" finds "Danish Superliga"); follow every game in one or browse its clubs. Unmapped leagues get a dynamic id (`l<idLeague>`), are registered on sight from fixtures, and show "where to watch unknown" rather than a guessed provider. Hand-mapped extras: Liga MX, Brasileirão, SPFL, Süper Lig, Argentine Primera, Saudi Pro League
